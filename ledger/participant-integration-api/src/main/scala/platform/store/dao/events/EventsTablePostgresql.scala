...
case class EventsTablePostgresql(idempotentEventInsertions: Boolean) extends EventsTable {
    ...
    for (((nodeId, node), i) <- info.events.zipWithIndex) {
      node match {
        case create: Create =>
          ...
          createKeyHashes(i) = serialized.createKeyHashes.get(nodeId).orNull
  ...
  private[events] val insertStmt = {
    ...
    s"""insert into participant_events(
           ...
           create_argument, create_signatories, create_observers, create_agreement_text, create_key_value, create_key_hash,
           exercise_choice, exercise_argument, exercise_result, exercise_actors, exercise_child_event_ids
         )
         select
           ...
           create_argument, string_to_array(create_signatories,'|'), string_to_array(create_observers,'|'), create_agreement_text, create_key_value, create_key_hash,
           exercise_choice, exercise_argument, exercise_result, string_to_array(exercise_actors,'|'), string_to_array(exercise_child_event_ids,'|')
         from
           unnest(
             ...
             {$createArguments}, {$createSignatories}, {$createObservers}, {$createAgreementTexts}, {$createKeyValues}, {$createKeyHashes},
             {$exerciseChoices}, {$exerciseArguments}, {$exerciseResults}, {$exerciseActors}, {$exerciseChildEventIds}
           )
           as
               t(
                 event_kind, event_id, event_offset, contract_id, transaction_id, workflow_id, ledger_effective_time, template_id, node_index, command_id, application_id, submitters, flat_event_witnesses, tree_event_witnesses,
